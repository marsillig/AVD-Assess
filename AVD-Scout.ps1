...
function Write-Banner {
    ...
    Write-Host '  |  https://virtex.cloud                        |' -ForegroundColor Cyan
...
<svg viewBox="0 0 130 130" width="$Size" height="$Size" class="donut" role="img" aria-label="$aria">
  <circle cx="65" cy="65" r="54" fill="none" stroke="#ece9f2" stroke-width="12"/>
...
:root { color-scheme: light; }
* { box-sizing: border-box; }
body { margin:0; font-family:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif; background:linear-gradient(180deg,#ece9f2 0%,#f6f7f9 42%,#fff 100%); color:#0a0a0a; line-height:1.5; -webkit-font-smoothing:antialiased; }
.container { max-width:1100px; margin:0 auto; padding:32px 24px 48px; }
header.hero { display:flex; align-items:center; justify-content:space-between; gap:24px; background:rgba(255,255,255,0.92); border:1px solid #ece9f2; border-radius:16px; padding:32px; margin-bottom:24px; }
.brand-name { font-size:34px; font-weight:800; letter-spacing:-0.02em; }
.brand-name .dot { color:#4e29a0; }
.brand-sub { font-size:13px; color:#3b3b3b; }
.hero-meta { text-align:right; font-size:12px; color:#3b3b3b; }
.hero-meta .big { font-size:30px; font-weight:800; color:#0a0a0a; }
.idx-card { background:rgba(255,255,255,0.92); border:1px solid #ece9f2; border-radius:16px; padding:22px 26px; margin-bottom:18px; }
.idx-head { font-size:17px; font-weight:700; }
.idx-count { display:inline-block; margin-left:8px; padding:1px 9px; border-radius:11px; font-size:12px; font-weight:700; color:#0a0a0a; background:#ece9f2; }
.idx-sub { font-size:13px; color:#3b3b3b; margin:4px 0 14px; }
.idx-list { list-style:none; margin:0; padding:0; }
.idx-list li { display:flex; align-items:baseline; gap:16px; padding:11px 0; border-top:1px solid #ece9f2; }
.idx-link { display:flex; align-items:baseline; gap:14px; flex:1; text-decoration:none; color:inherit; }
.idx-link:hover .idx-name { color:#4e29a0; }
.idx-name { font-size:15px; font-weight:600; color:#0a0a0a; }
